...
## 2026-09-02 — audit สองรอบบนงานของ 09-01: อะไรรอด อะไรไม่รอด

เจ้าของสั่ง spawn reviewer อิสระ 2 ตัว (opus) ตรวจงานของวันก่อน และประกาศหลักการ
**"ยุติธรรม ซื่อสัตย์"** เป็นหลักสูงสุดของโปรเจค เหนือกว่าเป้าหมายเอง — เขียนไว้บนสุด
ของ CLAUDE.md เหตุผลตามคำเจ้าของ: "เพราะไม่อยากให้ AI มั่ว"

รอบนี้จึงบันทึกทั้งที่ผิดและที่ถูก ตัวที่ผิดขึ้นก่อน

### ที่ผมเขียนผิด และแก้แล้ว

**1. เลขใน comment `entry floors` และใน CLAUDE.md ไม่มีคำสั่งไหนผลิตได้**

comment อ้าง `below floors n=99 peak-2x 21.2% rug 49.5% / above n=59 6.8% / 1.7%`
แต่ตารางที่มัน document อยู่:
- ใช้ `rug` = horizon return <= -90% ส่วน comment ใช้ `p.troughRet <= -0.9` (ต่างกัน ~10pp)
- ไม่ได้กรอง sampling density ส่วน comment กรอง
- แตกเป็น 2 ช่อง (`above floors, PASS` / `FAIL`) ไม่ใช่ช่องเดียว

และตัวที่แย่ที่สุด: `above floors n=59` **บังเอิญตรงกับช่อง PASS-only ที่พิมพ์ออกมา**
คนอ่านจะนึกว่า reproduce ได้แล้วทั้งที่คนละอย่าง

แก้โดย**ทำให้โค้ดพิมพ์เลขนั้นจริงๆ** ไม่ใช่แก้ comment: เพิ่มบล็อก `floors cost` ใน
`patterns` — dense rows only, พิมพ์ rug **ทั้งสองนิยาม**กำกับชื่อ, พิมพ์ median samples
ทั้งสองฝั่งให้ตรวจ sample-matching ได้เอง, และพิมพ์จำนวน distinct mint คู่กับ n
ไม่พิมพ์ p-value เพราะ split ที่โชว์เป็นหนึ่งในหลายอันที่ลอง

```
-- floors cost (dense rows only, samples >= 50, n=159) --
  below floors   n=100 (100 mints)  peak-2x 21.0%  rug(trough) 50.0%  rug(d3ret) 41.3% of 46  med samples 253
  above floors   n= 59 ( 59 mints)  peak-2x  6.8%  rug(trough)  1.7%  rug(d3ret)  2.8% of 36  med samples 259
    below: age only  n=49  peak-2x 24.5%  rug(trough) 61.2%
    below: liq only  n=13  peak-2x 15.4%  rug(trough)  0.0%
    below: both      n=38  peak-2x 18.4%  rug(trough) 52.6%
```

**2. "re-confirmed" — คำผิด**

CLAUDE.md เขียนว่า chg24h>1000 ถูก "re-confirmed inside both sampling-density strata"
ความจริง: มันคือการ**แบ่งแถวชุดเดิม 40 แถวใหม่** ยุคใหม่เพิ่มมา **8 แถว 1 event**
และ hot cell ของยุค pre-versioning คือ 11/40 — **ตรงบิตต่อบิตกับหัวข้อของ 08-31**
ไม่ใช่การยืนยันซ้ำ เกณฑ์ข้อ (ค) ของโปรเจคเอง (รอด stratify ตาม `v.ef`) **ใช้ไม่ได้เลย**
เพราะยุค `71c22712` ยังไม่มีแถวที่มี d3 สักแถว

**3. p-value ประกาศ convention ไม่ครบ**

reviewer รอบแรกกล่าวหาว่า "mixed tails" — **ผิด** auditor รอบสองยืนยันว่าเลขทั้ง 7 ตัว
เป็น one-tailed Fisher สม่ำเสมอ ไม่ได้ปนกัน

แต่คำวิจารณ์ที่**รอด**คือ: ไม่ได้ประกาศว่าใช้ tail ไหน และ one-tailed สมเหตุผลเฉพาะ
momentum kill ที่ pre-register ทิศไว้ ส่วน chg24h/vol-liq เป็น screen ที่ค้น threshold grid
one-tailed จะ inflate ทันที เทียบ:

| test | ที่รายงาน (1-tail) | 2-tail |
|---|---|---|
| momentum kill 1/20 vs 2/49 | 0.65 | 1.00 |
| entry floors 21/99 vs 4/59 | 0.012 | 0.023 |
| chg24h dense 8/27 vs 17/131 | 0.036 | 0.042 |
| vol/liq sparse 9/71 vs 3/81 | 0.040 | **0.067** |
| vol/liq dense 17/96 vs 8/62 | 0.28 | 0.51 |

**ไม่มีข้อสรุปไหนเปลี่ยน** ตัวเดียวที่ข้าม 0.05 คือ vol/liq sparse และมันข้ามไปในทาง
ที่**ยิ่งสนับสนุน**การลบ gate ที่ลบไปแล้ว

**นับจากนี้: exploratory screen ใช้ two-tailed, pre-registered directional test ใช้
one-tailed ได้แต่ต้องเขียนกำกับว่าเป็น one-tailed ทุกครั้ง**

**4. backtest -6.6% — เลขที่ป้องกันตัวได้น้อยที่สุด**

auditor สร้างใหม่ได้ -16.4% (mark ไม้ที่ยังไม่ปิดด้วยราคาล่าสุด) และ -28.5% (เฉพาะไม้ที่ปิดแล้ว)
-6.6% ของผมน่าจะมาจากการเครดิต OTC ที่ peak 6.79x ซึ่ง**กฎขายไม่ได้ผลิตการขายนั้น** —
"ขายครึ่งที่ 2x แล้วปล่อยที่เหลือวิ่ง" ไม่มีกฎขายสำหรับครึ่งหลังเลย ภายใต้กฎจริง OTC ออกที่ 2.86x

และ **"OTC แบกกำไรทั้งหมด" ก็เกินจริง** — 9 จาก 42 ไม้จบเหนือ 1.0x, OTC คิดเป็น 45%
ของส่วนเกินบวก ตัดออกแล้ว -16.4% -> -21.4%

**บทสรุปที่ซื่อสัตย์: ทิศทางแข็งแรง (ติดลบทุก variant ที่ทำตามกฎ) ขนาดไม่แข็งแรง
ห้ามเอาเลขตัวใดตัวหนึ่งไปเขียนใน CLAUDE.md เป็น "the figure"**

### ที่ผมถูก และ reviewer รอบแรกผิด

- **OTC 6.79x จากราคาแจ้งเตือน — ถูก** reviewer บอก 2.19x โดยอ่านจาก `r.s` ซึ่ง
  **หยุดตอน archive (d3 จากวันเจอ = 08-31 20:43)** ส่วนราคาเริ่มวิ่ง 08-31 19:24
  และ peak หลัง path หยุดไป 11 ชม. auditor ยืนยันจากแหล่งที่สาม (live fetch)
  **บทเรียนที่ควรจำ: หน้าต่าง d3 ปิดก่อน pump ได้ — `r.s` ไม่ใช่หลักฐานว่าเหรียญไม่วิ่ง**
- **"mixed tails" — ผิด** (ข้างบน)
- `fone` ไม่ใช่ peak สูงสุดใน dataset (อันดับ 7; PONS 73.7x, UMIA 65.2x นำอยู่)
  แต่**ประเด็นเรื่องเวลาถูก**และเป็นครึ่งที่สำคัญกว่า
- `gateFns` slice ที่ว่างเปล่าเป็นบั๊ก**เดิม** ไม่ใช่ของ commit เมื่อวาน

### ข้อค้นพบสำคัญที่สุดของรอบนี้ — alert gate สายโดยโครงสร้าง

auditor ยืนยันและทำให้แรงกว่าเดิม:

- 42 จาก 48 hot rows (chg24h>1000) อายุ < 48h
- **0 hot rows ผ่านทั้งสองด่าน AND pass** ภายใต้กฎวันนี้ (9 ตัวที่ pass ล้วนผ่านตอน
  minAgeHours ยัง 24)
- จำลอง `alertQualifies` ทั้ง predicate บนทุกแถว: เลือกได้ **9 แถว มี 0 จาก 37 pump events**
- `alertMaxChg24h: 100` vs `momMinChg24h: 1000` — **disjoint โดยนิยาม**
- **8 จาก 12 เหรียญที่ถูกเตือนและเคยแตะ 2x ถูกเตือน *หลัง* peak ของตัวเอง**
  (rehanfal +0.7h, Squirrel +1.6h, Polycat +2.3h, DOGE-1 +6.4h, PINK +11.2h,
  fone +90h, Pistacio +105h, Sue +127h) และมีแค่ 13 จาก 37 pump events ที่เคยถูกเตือนเลย

**กลไก**: `alertMaxChg24h: 100` ยอมให้ผ่านก็ต่อเมื่อ pump สลายลงต่ำกว่า +100%/24h แล้ว
ซึ่ง**โดยนิยามคือหลังจากมันวิ่งไปแล้ว** กฎนี้ทำงานตรงตามที่วัดมา (กันซื้อยอด) และราคาของมัน
คือความสายเชิงโครงสร้าง

แปลว่า: **การโปรโมท chg24h>1000 ไม่ใช่การขยับ threshold** มันคือการรื้อทั้งสองด่านพร้อมกับ
ล้มกฎที่มี return รองรับ ข้อเสนอใดๆ ที่จะโปรโมทต้องเขียนแบบนั้นใน STUDY.md และตีราคาทั้งสองฝั่ง
เขียนเข้า CLAUDE.md เป็น architecture note แล้ว

### re-entry stratum — เฝ้าดู ไม่ใช่ปิดกั้น

reviewer รอบแรกเสนอให้ใส่เพดาน liquidity ทันที **auditor รอบสองแย้ง และผมทำตาม auditor**

ตัวเลขที่ทั้งสองยืนยันตรงกัน: re-entry median liq $762k / FDV $9.5M / age 47d
เทียบ first-entry $94k / $774k / 7.7d — และ liq > $1M คือ **1/48 peak-2x (dense 0/11)**
เทียบ 14.0% ของ liq < $150k

แต่ auditor ชี้ว่า **pool ที่รอ cooldown อยู่ (20 mints) median liq = $92k มีเกิน $1M แค่ 1 ตัว**
แปลว่า skew คือ**คลื่นแรก**ที่ดูดผู้อยู่อาศัยถาวรตัวใหญ่ที่สุดของ trending_pools ไปก่อน
ไม่ใช่คุณสมบัติเชิงโครงสร้างของกลไก **ใส่เพดานตอนนี้ = tune กฎด้วยข้อมูลวันเดียว**
ซึ่งคือสิ่งที่ CLAUDE.md ห้าม

เลยเพิ่ม **re-entry stratum watch** ในหัว `patterns` แทน (median liq/FDV/age เทียบสองฝั่ง)
**pre-register: ประเมินอีกครั้งเมื่อ d3 ของ re-entry ชุดแรกสุก (~2026-09-04)** ถ้า median liq
ของ re-entry ยังสูงกว่า first-entry หนึ่ง order of magnitude ตอนนั้น -> เป็น RULES change ที่มีข้อมูลหนุน

ข้อที่ auditor หักล้าง counter-argument ของผมเอง: re-entry ไม่ได้เป็นแค่ control group —
**7 จาก 15 เป็น pass=true และในยุค `71c22712` first entries คือ 0 PASS/31 ส่วน re-entries
คือ 7 PASS/15** ทุกแถว PASS ใหม่ในยุคปัจจุบันเป็น re-entry ทั้งหมด การซ่อมท่อปลดล็อก PASS arm จริง
แต่ปลดล็อกเข้าไปในฝั่ง old/liquid ล้วน

### แก้อื่นๆ

- **`t-provenance.js` แดงมาตั้งแต่ 09-01 และไม่มีใครรู้เพราะไม่มีอะไรรันมัน**
  สาเหตุ: comment ที่ผมเพิ่มดัน `v: provenance()` ไปที่ offset 628 พ้นหน้าต่าง 600 ตัวอักษร
  โค้ดถูก test ผิด แก้เป็น brace-matching แทนการนับตัวอักษร
- **`gateFns` slice ว่างเปล่ามาตลอด** (`indexOf("function alertQualifies")` = 82634 อยู่
  *หลัง* `indexOf("function cmdWatch")` = 75819 -> slice = "") check นี้ผ่านบน string ว่าง
  ตั้งแต่วันที่เขียน แก้เป็นดึง function body ทีละตัวด้วย brace-matching + เพิ่ม guard
  ว่า slice ต้องยาว > 500 ตัวอักษร เพื่อไม่ให้บั๊กชนิดนี้กลับมาเงียบๆ ได้อีก
- **ใส่ `node t-provenance.js` เข้า workflow** (`continue-on-error` แล้ว re-raise ตอนท้าย —
  test แดงต้องไม่หยุดการเก็บข้อมูลบน schedule ที่เราแย่งมาไม่พออยู่แล้ว)
- **`alerts.jsonl` append-only** — `alerts-sent.json` เป็น cooldown cache ที่ prune ทุก
  4 x 72h = 288h **แต่มันเป็นหลักฐานเดียวว่าระบบเคยเตือนอะไรไปบ้าง** รายการแรกสุด (08-26)
  อีกไม่กี่วันจะถูกลบ = ทำลาย ground truth ของ backtest ทั้งหมด backfill 43 รายการเข้าไปแล้ว
  (ติด `backfilled: true` เพราะราคาเข้าเป็นการ reconstruct จาก snapshot ไม่ใช่ค่าที่บันทึกไว้จริง)
  ตั้งแต่นี้บันทึกราคา/liq/fdv/chg ณ ตอนเตือนลงไปตรงๆ — ความไม่แน่นอนตรงนี้คือสาเหตุที่
  backtest สองรอบต่างกัน 15 percentage points

### สิ่งที่ยังไม่รู้ และห้ามอ้างว่ารู้
